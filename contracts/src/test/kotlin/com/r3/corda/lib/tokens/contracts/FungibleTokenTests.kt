...
class FungibleTokenTests : ContractTestCommon() {
	...
	fun `move token tests`() {
		...
		transaction {
			...
			tweak {
				...
				command(BOB.publicKey, IssueTokenCommand(USD issuedBy BOB.party, outputIndexes = listOf(1)))
			...
			tweak {
				...
				command(ALICE.publicKey, MoveTokenCommand(USD issuedBy BOB.party, outputIndexes = listOf(1)))
			...
			tweak {
				...
				command(ALICE.publicKey, MoveTokenCommand(USD issuedBy BOB.party, inputIndexes = listOf(1)))
			...
			tweak {
				...
				input(FungibleTokenContract.contractId, 0.ofType(USD) issuedBy BOB.party heldBy ALICE.party)
				output(FungibleTokenContract.contractId, 10.ofType(USD) issuedBy BOB.party heldBy BOB.party)
				command(ALICE.publicKey, MoveTokenCommand(USD issuedBy BOB.party, inputIndexes = listOf(1, 2), outputIndexes = listOf(1)))
			...
			tweak {
				...
				output(FungibleTokenContract.contractId, 0.ofType(USD) issuedBy BOB.party heldBy BOB.party)
				command(ALICE.publicKey, MoveTokenCommand(USD issuedBy BOB.party, inputIndexes = listOf(1), outputIndexes = listOf(1, 2)))
				// Command for the move.
			...
			tweak {
				...
				command(ALICE.publicKey, MoveTokenCommand(USD issuedBy BOB.party, inputIndexes = listOf(1), outputIndexes = listOf(1)))
				// Command for the move.
			...
			tweak {
				...
				input(FungibleTokenContract.contractId, 20 of Ruble issuedBy CHARLIE.party heldBy CHARLIE.party)
				output(FungibleTokenContract.contractId, 10 of Ruble issuedBy CHARLIE.party heldBy CHARLIE.party)
				output(FungibleTokenContract.contractId, 10 of Ruble issuedBy CHARLIE.party heldBy CHARLIE.party)

				attachment(Ruble.importAttachment(aliceServices.attachments))

				command(CHARLIE.publicKey, MoveTokenCommand(GBP issuedBy CHARLIE.party, inputIndexes = listOf(1), outputIndexes = listOf(1)))
				command(CHARLIE.publicKey, MoveTokenCommand(Ruble issuedBy CHARLIE.party, inputIndexes = listOf(2), outputIndexes = listOf(2, 3)))
				verifies()
			}
			...
			tweak {
				attachment(Ruble.importAttachment(aliceServices.attachments))
				input(FungibleTokenContract.contractId, 20 of Ruble issuedBy CHARLIE.party heldBy CHARLIE.party)
				output(FungibleTokenContract.contractId, 20 of Ruble issuedBy CHARLIE.party heldBy DAENERYS.party)
				command(BOB.publicKey, MoveTokenCommand(Ruble issuedBy CHARLIE.party, inputIndexes = listOf(1), outputIndexes = listOf(1)))
	...
	fun `should prevent moving of tokens to a subclass`() {
		val issuedToken = Ruble issuedBy ISSUER.party
		...
		transaction {
			...
			tweak {
				...
				command(ALICE.publicKey, MoveTokenCommand(issuedToken, inputIndexes = listOf(0), outputIndexes = listOf(0)))
				command(BOB.publicKey, MoveTokenCommand(issuedToken, inputIndexes = listOf(1), outputIndexes = listOf(1)))
				verifies()
			}
		}
	}
	...
	fun `should be possible to redeem single token without change output`() {
		...
		transaction {
			...
			input(FungibleTokenContract.contractId, 10 of issuedToken heldBy ALICE.party)
			command(listOf(ISSUER.publicKey, ALICE.publicKey), RedeemTokenCommand(issuedToken, inputIndexes = listOf(0)))
			verifies()
		}
	}
	...
	fun `should be possible to redeem single token with change output`() {
		...
		transaction {
			...
			command(listOf(ISSUER.publicKey, ALICE.publicKey), RedeemTokenCommand(issuedToken, inputIndexes = listOf(0), outputIndexes = listOf(0)))
			verifies()
		}
	}
	...
	fun `should not be possible to redeem single token without owner signature`() {
		...
		transaction {
			// Start with a basic redeem which redeems 10 tokens in entirety from ALICE .
			input(FungibleTokenContract.contractId, 10 of issuedToken heldBy ALICE.party)
			command(listOf(ISSUER.publicKey), RedeemTokenCommand(issuedToken, inputIndexes = listOf(0)))
	...
	fun `should not be possible to redeem single token without issuer signature`() {
		...
		transaction {
			// Start with a basic redeem which redeems 10 tokens in entirety from ALICE .
			input(FungibleTokenContract.contractId, 10 of issuedToken heldBy ALICE.party)
			command(listOf(ALICE.publicKey), RedeemTokenCommand(issuedToken, inputIndexes = listOf(0)))
	...
	fun `should not be possible to redeem multiple tokens with different issuers without all issuer signatures`() {
		...
		transaction {
			...
			command(listOf(ALICE.publicKey, ISSUER.publicKey), RedeemTokenCommand(GBP issuedBy BOB.party, inputIndexes = listOf(1)))
			this `fails with` "The issuer must be the signing party when an amount of tokens are redeemed."
		}
	}
	...
	fun `should allow redemption of two separate groups`() {
		...
		transaction {
			...
			input(FungibleTokenContract.contractId, 10 of GBP issuedBy BOB.party heldBy ALICE.party)
			command(listOf(ALICE.publicKey, ISSUER.publicKey), RedeemTokenCommand(issuedToken, inputIndexes = listOf(0)))
			command(listOf(ALICE.publicKey, BOB.publicKey), RedeemTokenCommand(GBP issuedBy BOB.party, inputIndexes = listOf(1)))
			verifies()
		}
	}
	...
	fun `should not allow redemption if more than one change output is present`() {
		...
		transaction {
			...
			command(listOf(ISSUER.publicKey, ALICE.publicKey), RedeemTokenCommand(issuedToken, inputIndexes = listOf(0), outputIndexes = listOf(0, 1)))
	...
	fun `should not allow redemption if no input states present`() {
		...
		transaction {
			// Start with a basic redeem which redeems 10 tokens in entirety from ALICE .
			output(FungibleTokenContract.contractId, 1 of issuedToken heldBy BOB.party)
			command(listOf(ISSUER.publicKey, ALICE.publicKey), RedeemTokenCommand(issuedToken, outputIndexes = listOf(0)))
...
//			}
	...
	fun `should enforce that the hash providing the token type cannot change during a transaction`() {
		...
		transaction {
			...
			output(FungibleTokenContract.contractId, FungibleToken(10 of issuedToken, BOB.party, Ruble.getAttachmentIdForGenericParam()))
			command(BOB.party.owningKey, MoveTokenCommand(issuedToken, inputIndexes = listOf(0), outputIndexes = listOf(0)))
