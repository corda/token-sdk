...
//        val ourSummary = describeTransaction(inputs.map{ it.state}, outputs.map { it.state }, moveCommands.map { it.value }, attachments.map { it.id })
    ...
    override fun describeTx(tx: BaseTransaction): List<String> {
        return if (tx.inputs.isNotEmpty() && tx.outputStates.isNotEmpty()) {
//            val serializedResolvedInputs = tx.inputs.map { ref ->
...
//            val resolvedInputs = serializedResolvedInputs.lazyMapped { star, _ -> star.toStateAndRef() }
        val inputStates = tx.inputs as List<StateAndRef<ContractState>>
        val moveFrom = inputStates.first().asFungibleToken().holder.owningKey
        val moveTo = tx.outputStates.first().asFungibleToken().holder.owningKey

        val amountToMove = tx.outputStates.first { p -> p.asFungibleToken().holder.owningKey != moveFrom }.asFungibleToken().amount
        listOf("Move ${amountToMove.toDecimal()} ${amountToMove.token.tokenType.tokenIdentifier} from key: ${moveFrom.toStringShort()} to key: ${moveTo.toStringShort()}")
        } else {
            emptyList()
        }
    }
    ...
    override fun describeTransaction(
        ...
        return when(commands.first()) {
            ...
            is IssueTokenCommand -> emptyList()
    ...
    private fun StateAndRef<ContractState>.asFungibleToken() = this.state.data as FungibleToken
    private fun ContractState.asFungibleToken() = this as FungibleToken
}
