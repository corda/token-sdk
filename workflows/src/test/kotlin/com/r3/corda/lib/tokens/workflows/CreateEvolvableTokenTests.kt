...
class CreateEvolvableTokenTests : JITMockNetworkTests() {
   ...
   */
	...
	fun `should not update EvolvableToken by observers`() {
		...
		val house =
			House("24 Leinster Gardens, Bayswater, London", 1_000_000.GBP, listOf(denise.legalIdentity(), alice.legalIdentity()), linearId = UniqueIdentifier())
	...
	*/
	...
	fun `should remove maintainer from maintainers and the party should not be able to update anymore`() {
		...
		val house =
			House("24 Leinster Gardens, Bayswater, London", 900_000.GBP, listOf(alice.legalIdentity(), bob.legalIdentity()), linearId = UniqueIdentifier())
		// Empty observer list
		...
		val newHouseToken1 =
			House("24 Leinster Gardens, Bayswater, London", 855_000.GBP, listOf(alice.legalIdentity(), bob.legalIdentity()), linearId = createdTokenId)
		...
		val newHouseToken3 =
			House("24 Leinster Gardens, Bayswater, London", 860_000.GBP, listOf(alice.legalIdentity()), linearId = updateTxByNodeAState.linearId())
