...
class VaultWatcherServiceTest {
	...
	fun `should remove states which have been spent`() {
		...
		val selectedTokensAfterSpend =
			vaultWatcherService.selectTokens(Holder.KeyIdentity(owner), Amount(10000000000, GBP), allowShortfall = true, selectionId = "abc")
	...
	fun `should allow selection by multiple holder types`() {
		...
		for (accountsAndKeyEntry in accountsAndKeys) {
			...
			val selectedTokens =
				vaultWatcherService.selectTokens(Holder.MappedIdentity(account), Amount(10000000000, GBP), allowShortfall = true, selectionId = "CHEESEY_BITES")
		...
		for (keyToTokenEntry in keyToTokenMap) {
			...
			val selectedTokens =
				vaultWatcherService.selectTokens(Holder.KeyIdentity(key), Amount(10000000000, GBP), allowShortfall = true, selectionId = "CHEESEY_BITES")
					.sortedBy { it.toString() }
	...
	companion object {
		...
		fun getDefaultVaultObserver(): Pair<TokenObserver, PublishSubject<Vault.Update<FungibleToken>>> {
			val observable = PublishSubject.create<Vault.Update<FungibleToken>>()
			return Pair(
				TokenObserver(listOf(), uncheckedCast(observable), { stateAndRef, _ -> Holder.KeyIdentity(stateAndRef.state.data.holder.owningKey) }),
				observable
			)
		}
	}
