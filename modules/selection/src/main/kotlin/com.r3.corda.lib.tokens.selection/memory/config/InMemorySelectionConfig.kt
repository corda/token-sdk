	...
	companion object {
		...
		fun parse(config: CordappConfig): InMemorySelectionConfig {
			...
			val indexingType = try {
				val list = uncheckedCast<Any, List<Any>>(config.get("stateSelection.inMemory.indexingStrategies"))
				list.map { VaultWatcherService.IndexingType.valueOf(it.toString()) }
